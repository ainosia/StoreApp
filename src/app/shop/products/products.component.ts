...
export class ProductsComponent implements OnInit {
  ...
  ngOnInit() {
    this.products = this.productsService.products().snapshotChanges().map(productSnapsh => {
      return productSnapsh.map(product => {
        const productData = product.payload.doc.data();
        const productId = product.payload.doc.id;
        return this.productsService.getProductImages(productId).snapshotChanges().map(uploadSnap => {
          let number = 0;
