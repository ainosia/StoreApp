...
export class CartComponent implements OnInit {
  ...
  update(product: Product, qty) {
    ...
    this.cartService.updateProduct(product, qty.value, this.uid).then(() => {
      this.snackService.launch("Producto actualizado", "Cesta", 4000);
      this.appService.stopLoader();
    })
    .catch(() => {
      this.snackService.launch('Error actualizando el producto', 'Cesta', 4000);
      this.appService.stopLoader();
    });
  }
  ...
  remove(product: Product) {
    ...
    this.cartService.removeProduct(product, this.uid).then(() => {
      this.snackService.launch("Producto eliminado", "Cesta", 4000);
      this.appService.stopLoader();
    })
      .catch(() => {
        this.snackService.launch('Error eliminando el producto', 'Cesta', 4000);
        this.appService.stopLoader();
      });
  }
  ...
  processOrder() {
    ...
    this.ordersService.save(order).then(() => {
      this.cartService.resetCart(this.uid).then(() => {
        this.snackService.launch('Carrito eliminado y pedido creado', 'Cesta', 6000);
